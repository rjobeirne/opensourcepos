...
class Reports extends Secure_area
{
	...
	function summary_sales($start_date, $end_date, $sale_type)
	{
		...
		$data = array(
			...
			"data" => $tabular_data,
			'expense_sum' => $model->DataSum1(array(
			'start_date'=>$start_date,
			'end_date'=>$end_date,
			'sale_type' => $sale_type)),
	...
	function summary_expense_categories($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		$expense_sum = $model->DataSum(array(
			'start_date'=>$start_date,
			'end_date'=>$end_date,
			'sale_type' => $sale_type));
		...
		foreach($expense_sum as $row)
		{
			$tabular_data[] = array(
			 $row['Totalcount'],
			to_currency($row['totlsumexpense'])
			);
		}
		$data = array(
		"title" => $this->lang->line('reports_expense_categories_summary_report'),
	...
	// expense category
	//Graphical 
	function graphical_summary_expense_categories($start_date, $end_date, $sale_type)
	{
		$this->load->model('reports/Summary_expenses_categories');
		$model = $this->Summary_expenses_categories;
	...
	function graphical_summary_expense_categories_graph($start_date, $end_date, $sale_type)
	{
		$this->load->model('reports/Summary_expenses_categories');
		$model = $this->Summary_expenses_categories;
